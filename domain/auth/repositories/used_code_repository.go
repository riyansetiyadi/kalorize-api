...
func (db *UsedCode) DeleteUsedCode(idUsedCode string) error {
	return db.Conn.Delete(&models.UsedCode{}, idUsedCode).Error
}

func (db *UsedCode) GetUsedCodeByIdCode(idUsedCode string) (models.UsedCode, error) {
	var usedCode models.UsedCode
	err := db.Conn.Where(" id_kode = ?", idUsedCode).First(&usedCode).Error
	return usedCode, err
}

func (db *UsedCode) GetusedCodeByIdUser(idUser string) (models.UsedCode, error) {
	var usedCode models.UsedCode
...
type UsedCodeRepository interface {
	...
	DeleteUsedCode(idUseCode string) error
	GetUsedCodeByIdCode(idUseCode string) (models.UsedCode, error)
	GetusedCodeByIdUser(idUser string) (models.UsedCode, error)
}
